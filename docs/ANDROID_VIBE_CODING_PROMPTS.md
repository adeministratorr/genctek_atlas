...
**Açıklama:**
Bu prompt, etkinlik ve proje gönderme ekranlarını mobil dikey düzene uygun olarak Stitch tasarım aracında tasarlatır.

**Prompt:**

```text
GençTek Atlas Android uygulaması için dikey kaydırılabilir mobil uyumlu 'Etkinlik Ekleme Formu' ve 'Proje Ekleme Formu' arayüzlerini tasarla.

1. Mobil Uyumlu Etkinlik Formu (Stitch):
   - Dikey kaydırılabilir (scrollable) tek sütun düzeni.
   - Form alanları: Etkinlik Adı, Kapsam (İl, İlçe, Okul, Türkiye - Seçim kutusu), Durum (Gerçekleşti, Ön Duyuru - Seçim kutusu), Tema, Tarih ve Özet Açıklama (Maks. 500 karakter sayaçlı).
   - Konum Seçimi: Harita yer tutucusu (Google Maps Android SDK placeholder) ve altında otomatik koordinat (enlem/boylam) kutuları.
   - Duruma Göre Dinamik Paneller:
     * 'Ön Duyuru' seçildiğinde: Kontenjan sınırı alanı ve İl/İlçe başvuru kısıtlama onay kutuları.
     * 'Gerçekleşti' seçildiğinde: Katılan okulların listelendiği, okul adı ve katılımcı sayısının dinamik satır ekle/sil butonlarıyla yönetildiği mobil tablo kartı.
   - Fotoğraf Yükleme: Sürükle-bırak/tıkla-seç formatında mobil uyumlu Kapak Görseli ve Galeri önizleme alanı.

2. Mobil Uyumlu Proje Formu (Stitch):
   - Tek sütunlu dikey yerleşim.
   - Giriş alanları: Proje Adı, Takım Adı, Parkur/Kategori, İlişkili Etkinlik, Tema ve Katılımcı İller (etiketli metin alanı).
   - Bağlantılar: GitHub (GitHub logolu) ve Demo Canlı bağlantısı ile Proje Açıklaması (Maks. 1000 karakter sayaçlı).
   - Dosya Yüklemeleri: Ekran Görüntüsü (Görsel) ve Prompt Belgesi (.md/.txt) için mobil dosya yükleyici kartları.
   - Etik Kontrol Kartı: En altta kalkan (Shield) ikonu içeren, 'Etik ve AI Kullanım Sözleşmesini Onaylıyorum' metnine sahip geniş dokunma alanlı kart. (Tema Vibe Coding seçildiğinde bu kartın kırmızı bir sınır çizgisiyle vurgulanmasını görselleştir).

Tasarımda kurumsal kırmızı-beyaz temayı kullan ve butonları mobil dokunma alanına uygun tasarla.
```
...
## 9. Tasarımları Dışa Aktarma (Stitch'ten Çıktı Alma)

**Açıklama:**
Stitch'te sadece UI/UX tasarımını ve tasarım sistemini ürettikten sonra, bu tasarımları kodlamaya hazırlamak için dışa aktarmamız gerekir. Stitch'ten HTML/CSS olarak alınan tasarım, Android'de WebView olarak kullanılmamalıdır; bunun yerine AI Studio'ya görsel bir "referans" olarak verilmelidir.

**Nasıl Yapılır?**
1. Stitch'te tasarımları tamamladıktan sonra **Tasarım Dışa Aktar** (Export) seçeneğine tıkla.
2. Eğer **AI Studio ile Geliştir** (Export to AI Studio) seçeneği varsa doğrudan AI Studio'ya gönder.
3. Yoksa, ekranların **HTML/CSS kodlarını** ve **DESIGN.md** (veya tasarım sistemi özetini) bilgisayarına indir.

---

## 10. Google AI Studio ile Native Kodlama

**Açıklama:**
Stitch'ten gelen tasarımları referans alarak Google AI Studio'da tamamen native (Kotlin + Jetpack Compose) bir Android projesi oluşturacağız. En büyük hata tasarımı WebView içine gömmektir. Amacımız tasarımı referans alıp baştan native Android bileşenleriyle örmektir.

**Nasıl Yapılır?**
1. AI Studio'da proje oluştururken hedef olarak **Build an Android App** (Android Uygulaması Oluştur) seçeneğini işaretle.
2. Stitch çıktılarını (DESIGN.md, HTML/CSS kodları) AI Studio'ya ekle.
3. Aşağıdaki prompt'u kopyala ve gönder.

**Prompt (AI Studio İçin):**

```text
Google Stitch'te oluşturulmuş bir UI tasarımım var. Bu tasarımı native bir Android uygulamasına dönüştür.

Önemli:
- Kesinlikle WebView oluşturma.
- HTML/CSS kodlarını doğrudan gömme.
- Arayüzü Kotlin, Jetpack Compose, Material 3 ve Android en iyi pratiklerini kullanarak tamamen native olarak yeniden kur.
- Ekteki/dışa aktarılan Stitch HTML/CSS kodunu, DESIGN.md dosyasını ve ekran açıklamalarını sadece görsel ve UX referansı olarak kullan.
- Uygulamada görünen tüm metinler Türkçe olmalıdır. Ancak kod içi tanımlayıcıları, bileşen isimlerini ve dosya adlarını İngilizce tut.
...
- Material 3
- Navigation Compose
- MVVM mimarisi (ViewModel + StateFlow)
- Örnek veriler için Mock repository
- Temiz paket (package) yapısı
...
1. Giriş/Kayıt
2. Ana Keşfet Ekranı
3. Ekle/Düzenle Formu
4. Gruplar ve Görevler
5. Profil

Tasarım sadakati gereksinimleri:
- Stitch'teki görsel hiyerarşiyi koru
- Renkleri, kart yapılarını, boşlukları, tipografi stilini ve navigasyon yapısını koru
- Native Compose bileşenleri kullan
- Uygulamayı yaygın Android telefon boyutlarında duyarlı (responsive) yap
- Yükleniyor (loading), boş (empty) ve hata (error) durumlarını ekle

Geliştirme stratejisi:
1. İlk olarak çalışan bir native Android uygulama iskeleti kur.
2. Ardından navigasyon yapısını uygula.
3. Daha sonra her bir ekranı örnek (mock) verilerle oluştur.
4. Son olarak, Compose arayüzünü Stitch tasarımına daha çok benzeyecek şekilde (boşluklar, kart köşeleri, tipografi boyutları, renkler vb.) iyileştir.
5. Lütfen Kotlin dosyalarını tek tek gönder ve her birinin hangi pakete ait olduğunu açıkla.
```

**İpuçları (3 Aşamalı Strateji):**
- **Aşama 1 (Stitch):** Sadece UI/UX tasarımı üretilir (Arka plan mantığı düşünülmez).
- **Aşama 2 (AI Studio):** Yukarıdaki prompt ile çalışan, temiz mimarili bir Android iskeleti (skeleton) kurulur ve temel ekranlar üretilir.
- **Aşama 3 (AI Studio / Android Studio):** İskelet çalıştıktan sonra *"Şimdi Compose arayüzünü Stitch tasarımına daha çok benzeyecek şekilde iyileştir"* denilerek piksel/polish (tasarım benzerliği) iyileştirmesi yapılır. AI Studio'dan çıkan projeyi Android Studio'ya indirebilir veya tarayıcıdaki emülatör üzerinden test edebilirsin.
```

---

## 11. Android Proje Kurulumu
...
## 12. Tema, Bileşenler ve Navigasyon Kodlama
...
## 13. Firebase Bağlantısı ve Demo Modu
...
## 15. Giriş, Kayıt ve Rol Kontrolü Kodlama
...
## 16. Keşfet, Harita ve Filtre Kodlama
...
## 17. Etkinlik ve Proje Formları Kodlama

**Açıklama:**
Bu prompt, Stitch tasarımlarını referans alarak Etkinlik ve Proje formlarını Jetpack Compose ile mobil uyumlu ve validasyonlu olarak kodlatır.

**Prompt:**

```text
Yüklediğim Stitch tasarımlarındaki mobil uyumlu 'Etkinlik Ekleme Formu' ve 'Proje Ekleme Formu' ekranlarını Jetpack Compose kullanarak tek sütunlu (single-column) ve dikey kaydırılabilir (scrollable Column) yapıda kodla.

İstediğim Davranış ve Validasyonlar:
1. Kullanıcı giriş yapmamışsa form sayfalarına erişemesin (Giriş ekranına yönlendir).
2. Giriş alanlarında zorunlu alan doğrulaması yap ve hataları Türkçe göster.
3. GitHub linki 'https://github.com/' ile başlamak zorundadır. Demo linki geçerli bir URL olmalıdır.
4. Dosya Yükleme: Kapak görseli/ekran görüntüsü (maks 5MB) ve prompt dosyası (maks 2MB, sadece .md/.txt) için Android ActivityResultContracts.GetContent() API entegrasyonunu bağla.
5. Vibe Coding Etik Onay Kontrolü: Tema 'vibe-coding' seçildiğinde 'etikKontrol' checkbox'ının işaretlenmesi zorunlu olsun.
6. Yeni kayıtları Firestore'a varsayılan olarak durum = "pending" (beklemede) olacak şekilde kaydet.
7. Klavyenin açılma durumuna göre form alanlarının görünür kalmasını sağla (`Modifier.imePadding()` kullan).
8. Demo Modu aktifse, yeni kayıtları yerel geçici bir listeye ekle ve Firebase'e yazma.

Gerekli dosya ve klasörleri sen oluştur. Koddan sonra hangi validasyonları ve mobil klavye ayarlarını yaptığını açıkla.
```

---

## 18. Gruplar, Görevler ve Duyurular Kodlama
...
## 19. Mesajlaşma, Bildirim ve Profil Kodlama
...
## 21. Firestore ve Storage Güvenlik Kuralları
...
## 24. Derleme, APK ve Yayına Hazırlık
